...
func (c *Config) validate() error {
	...
	// cluster.rpc_urls containing the local validator RPC URL is allowed but requires
	// that HA peers are configured as mutual --entrypoint flags in agave for direct gossip.
	// Without direct entrypoints, local RPC may have stale gossip data about peers.
	...
	for _, clusterRPCURL := range c.Cluster.RPCURLs {
		...
		if clusterRPCHost == validatorRPCHost {
			c.logger.Warnf("cluster.rpc_urls contains local validator RPC URL (%s) - ensure HA peers are configured as mutual --entrypoint flags for direct gossip, otherwise peer state may be stale", c.Validator.RPCURL)
			break
		}
	}
