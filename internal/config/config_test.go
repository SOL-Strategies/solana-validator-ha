...
func TestValidate(t *testing.T) {
	...
	// Test with local RPC in cluster.rpc_urls (allowed with warning, requires mutual entrypoints)
	cfg.Validator.RPCURL = "http://localhost:8899"
	cfg.Cluster.RPCURLs = []string{"http://localhost:8899", "https://api.testnet.solana.com"}
	err = cfg.validate()
	assert.NoError(t, err) // Now allowed (warning logged, not error)

	// Test with different ports on same host (allowed, different endpoint)
	cfg.Validator.RPCURL = "http://localhost:8899"
	cfg.Cluster.RPCURLs = []string{"http://localhost:8900"}
	err = cfg.validate()
	assert.NoError(t, err)

	// Test with same host:port in cluster.rpc_urls (allowed with warning)
