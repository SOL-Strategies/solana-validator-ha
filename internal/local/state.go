...
func (s *State) SampleSelf() {
	...
	if healthy {
		if s.healthySince.IsZero() {
			...
			if !firstSample {
				s.logger.Info(fmt.Sprintf("self healthy for %s < %s minimum healthy duration - recovering",
					time.Since(s.healthySince).Round(time.Second),
		...
		if !s.unhealthyLogged {
			if healthErr != nil {
				s.logger.Error("self unhealthy", "error", healthErr)
			} else {
				s.logger.Error("self unhealthy")
			}
